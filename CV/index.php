...
$work[$key]['work_desc']=$action->db->clean(str_replace("\r\n", "", $_POST['work_desc'][$key]));
    ...
    $resume_data[7] = json_encode($certificate);
    $resume_data[8] = json_encode($refer);
    $resume_data[9] = json_encode($_POST['userinfo']);
    $resume_data[10] = $action->helper->UID();
    $run=$action->db->insert('resumes', 'user_id,headline,objective,contact,skills,experience,education,certificate,reference,addinfo,url', $resume_data);
    if($run)
    {
